...
def main():
    ...
    creator.create("Assignments", list)
    creator.create("Truck", Truck)

    IND_SIZE = 10

    toolbox = base.Toolbox()
    toolbox.register("assignment", random.choice, assignments)
    toolbox.register("truck", random.choice, trucks)
    toolbox.register("individual", creator.Individual, toolbox.truck, toolbox.assignment)
    toolbox.register("population", tools.initRepeat, list, toolbox.individual)

    toolbox.register("mate", tools.cxTwoPoint)
    toolbox.register("mutate", tools.mutGaussian, mu=0, sigma=1, indpb=0.1)
    toolbox.register("select", tools.selTournament, tournsize=3)
    toolbox.register("evaluate", Loading.evaluate, toolbox.individual)
